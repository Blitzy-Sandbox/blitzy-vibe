...
_DEFAULT_VIBE_HOME = Path.home() / ".blitzy"


def _get_vibe_home() -> Path:
    if vibe_home := os.getenv("BLITZY_HOME"):
...
LOG_FILE = GlobalPath(lambda: VIBE_HOME.path / "blitzy.log")

DEFAULT_TOOL_DIR = GlobalPath(lambda: VIBE_ROOT / "core" / "tools" / "builtins")
BUNDLED_SKILLS_DIR = GlobalPath(lambda: VIBE_ROOT / "skills" / "bundled")
