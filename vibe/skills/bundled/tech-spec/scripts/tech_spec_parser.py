            ...
            if child.type == "atx_heading":
                ...
                if match:
                    ...
                    feature = FeatureEntry(
                        ...
                        has_overview="**overview**" in section_lower
                        or "overview:" in section_lower,
                        has_business_value="**business value**" in section_lower
                        or "business value:" in section_lower,
                        has_user_benefits="**user benefits**" in section_lower
                        or "user benefits:" in section_lower,
                        has_technical_context="**technical context**" in section_lower
                        or "technical context:" in section_lower,
                        has_dependencies="prerequisite features" in section_lower
                        or "dependency type" in section_lower,
...
class ComponentVisitor(NodeVisitor):
    ...
    def _extract_responsibility(self, text: str) -> str:
        ...
        match = re.search(
            r"Purpose and Responsibilities?\s*\n+(.+?)(?:\n\n|\n###|\n\|)",
            text,
...
class TableVisitor(NodeVisitor):
    ...
    def _is_tech_table(self, headers: list[str]) -> bool:
        return (
            "component" in headers or "technology" in headers
        ) and "details" in headers

    def _is_integration_table(self, headers: list[str]) -> bool:
        return (
            "system" in headers or "system name" in headers
        ) and "integration" in " ".join(headers)
    ...
    def _extract_tech_entries(self, table_node, headers, doc):
        ...
        for child in table_node.children:
            ...
            component = (
                cells[comp_idx].strip()
                if comp_idx >= 0 and len(cells) > comp_idx
                else ""
            )
            technology = (
                cells[tech_idx].strip()
                if tech_idx >= 0 and len(cells) > tech_idx
                else ""
            )
            details = (
                cells[details_idx].strip()
                if details_idx >= 0 and len(cells) > details_idx
                else ""
            )

            if component or technology:
                doc.technologies.append(
                    TechnologyEntry(
                        component=component, technology=technology, details=details
                    )
                )
    ...
    def _extract_integration_entries(self, table_node, headers, doc):
        ...
        for child in table_node.children:
            ...
            system = (
                cells[sys_idx].strip() if sys_idx >= 0 and len(cells) > sys_idx else ""
            )
            int_type = (
                cells[int_idx].strip() if int_idx >= 0 and len(cells) > int_idx else ""
            )
            data_ex = (
                cells[data_idx].strip()
                if data_idx >= 0 and len(cells) > data_idx
                else ""
            )

            if system:
                doc.integrations.append(
                    IntegrationEntry(
                        system=system, integration_type=int_type, data_exchange=data_ex
                    )
                )
...
class ScopeVisitor(NodeVisitor):
    ...
    def visit(self, node, doc: TechSpecDocument, source_lines: list[str]) -> None:
        ...
        is_out_scope = (
            "out-of-scope" in title_text
            or "out of scope" in title_text
            or "excluded" in title_text
        )
...
class TechSpecParser:
    ...
    def format_output(self, doc: TechSpecDocument) -> str:
        ...
        lines.append(
            f"- **Features (F-xxx)**: {doc.feature_count} ({doc.complete_features} complete)"
        )
        ...
        lines.append(
            f"- **Scope items**: {len(doc.scope_items)} ({doc.in_scope_count} in-scope, {doc.out_scope_count} out-of-scope)"
        )
        ...
        lines.append(
            f"**Coverage**: {len(present)}/{len(EXPECTED_SECTIONS)} expected sections found"
        )
        lines.append("")
        ...
        if doc.features:
            ...
            lines.append("")
            lines.append(
                "| Feature ID | Name | Category | Priority | Status | Complete |"
            )
            lines.append(
                "|------------|------|----------|----------|--------|----------|"
            )
            for f in doc.features:
                ...
                lines.append(
                    f"| {f.feature_id} | {f.name[:40]}{'...' if len(f.name) > 40 else ''} | {f.category or '-'} | {f.priority or '-'} | {f.status or '-'} | {complete} |"
                )
            lines.append("")
            ...
            if incomplete:
                ...
                for f in incomplete:
                    ...
                    lines.append(
                        f"- **{f.feature_id}**: Missing {', '.join(missing_attrs)}"
                    )
                lines.append("")
        ...
        if doc.technologies:
            ...
            for t in doc.technologies:
                lines.append(
                    f"| {t.component} | {t.technology} | {t.details[:50]}{'...' if len(t.details) > 50 else ''} |"
                )
            lines.append("")
        ...
        if doc.integrations:
            ...
            for i in doc.integrations:
                lines.append(
                    f"| {i.system} | {i.integration_type} | {i.data_exchange[:40]}{'...' if len(i.data_exchange) > 40 else ''} |"
                )
            lines.append("")
...
def main():
    ...
    parser.add_argument("file", type=str, help="path to the tech spec markdown file")
    parser.add_argument(
    ...
    config = TechSpecConfig(
        file_path=str(file_path), verbose=args.verbose, focus=args.focus
    )
